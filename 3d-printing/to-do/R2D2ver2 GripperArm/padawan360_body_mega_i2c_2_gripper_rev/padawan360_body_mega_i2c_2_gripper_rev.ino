...
int LeftDoorOpen=180;
int LeftDoorClose=487;
int RightDoorOpen=440;
int RightDoorClose=180;
...
/////////////////////////////////////////////////////////////////
Sabertooth Sabertooth2x(128, Serial3);
...
//MB: Holoprojector amimaton variables...
 ...
 int GripperAni1a = 0; //Holo animation counter
...
void setup() {
  Serial3.begin(SABERTOOTHBAUDRATE);
...
void loop() {
  ...
  // OPEN Breadpan BOTH DOORS WHEN PRESSING RIGHT
if (Xbox.getButtonClick(UP, 0)) {
    // volume up
...
{ 
  pwm1.setPWM(HoloPWMstart+2,0,0); // lights off
  pwm1.setPWM(HoloPWMstart+3,0,0); //lights off
 pwm1.setPWM(HoloPWMstart, 0, Holo1_1); //Servo1 home
 pwm1.setPWM(HoloPWMstart+1,0,Holo2_1); //Servo2 home
   ...
   {
 pwm1.setPWM(HoloPWMstart, 0, Holo1_2); //Servo1
 pwm1.setPWM(HoloPWMstart+1,0,Holo2_2); //Servo2
   }

   else if (HoloAni1a <800) // Event 4
   {
 pwm1.setPWM(HoloPWMstart, 0, Holo1_1); //Servo1
 pwm1.setPWM(HoloPWMstart+1,0,Holo2_1); //Servo2
   }

   else if (HoloAni1a<850) //Event 3
   {
 pwm1.setPWM(HoloPWMstart, 0, Holo1_3); //Servo1
 pwm1.setPWM(HoloPWMstart+1,0,Holo2_3); //Servo2
   }

   else if (HoloAni1a<900) //Event 2
   {
 pwm1.setPWM(HoloPWMstart, 0, Holo1_4); //Servo1
 pwm1.setPWM(HoloPWMstart+1,0,Holo2_4); //Servo2
   }

   else if (HoloAni1a<980) //Event 1
   {
   pwm1.setPWM(HoloPWMstart+2,0,4095); // lights on
  pwm1.setPWM(HoloPWMstart+3,0,4095); //lights on
   }
...
if(GripperAni1a > 0) //Check to see if the animation loop has started
{
 ...
 GripperAni1a--;
    ...
    pwm2.setPWM(1,0,RightDoorClose);  

    }

    else if (GripperAni1a <150) // //Arm in
    {
    pwm2.setPWM(2,0,GripperArmIn);
    }
